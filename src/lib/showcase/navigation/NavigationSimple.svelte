...
*/
	...
	/*export const currentRoute = writable(menuContent.getLastRoute())

	let
		navMenuByThis,
		navMenuByConstructor,
		navMenuOpenedOnMobile: boolean = true

	onMount(async () => {
		if (window.location.pathname.length > 1) {
		}
		currentRoute.set(menuContent.findRouteFromPath(window.location.pathname))
		if (!history.state)
			window.history.replaceState({path: window.location.pathname}, '', window.location.href)
		navMenuByConstructor = new Collapse(navMenuByThis)
	})

	function redirectTo(event) {
		const chosenPath = event.target.pathname
		// change current router path
		currentRoute.set(menuContent.findRouteFromPath(chosenPath))
		// push the path into web browser history API
		window.history.pushState({path: chosenPath}, '', window.location.origin + chosenPath)
		if (navMenuOpenedOnMobile) {
			navMenuByConstructor.hide()
			navMenuOpenedOnMobile = false
		}
	}

	function handlerBackNavigation(event) {
		currentRoute.set(menuContent.findRouteFromPath(event.state.path))
	}
...
<nav>
	<!--<nav class="navbar navbar-expand-md d-md-flex flex-md-wrap flex-column p-0 border-end overflow-scroll">-->
	...
	<div>
		<!--<div class="collapse navbar-collapse me-auto" id="navbarToggleExternalContent" bind:this={navMenuByThis}>-->
			...
			{#each menuContent.chapters as chapter}
				<li class="mb-1">
					<!--<button class="btn btn-toggle align-items-center collapsed"
									...
									aria-expanded="true"
									on:click|preventDefault={redirectTo}>-->
...
-->
					<div>
						<!--<div class="collapse show" style="" id={chapter.ownRoute.pathLettersOnly + '-nav-collapse'}>-->
						<ul class="list-unstyled">
